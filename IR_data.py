...
def read_data_sets(total_samples=None, train_file="./IR_train_data_temp.npz",
                    test_file="./IR_test_data_temp.npz"):

    IMG_PATH_TRAIN = "D:/MultiRobotsPer/youtube/2017-12-06/YoutubeCutDone/2017-12-12-IR-All/"
    IMG_PATH_TEST = "D:/MultiRobotsPer/youtube/2017-12-06/YoutubeCutDone/2017-12-12-IR-KnownBackground/"
    ...
    train_files = [os.path.basename(x) for x in glob.glob(os.path.join(IMG_PATH_TRAIN, "*"))]
    test_files = [os.path.basename(x) for x in glob.glob(os.path.join(IMG_PATH_TEST, "*"))]

    train_images, train_labels = files_to_images(train_files, total_samples, IMG_PATH_TRAIN, train_file)
    test_images, test_labels = files_to_images(test_files, total_samples, IMG_PATH_TEST, test_file)

    return split_into_datasets(train_images, train_labels, test_images, test_labels)

def get_image_info(image_name):
    info = image_name.split(".")[0].split("_")
    return {"label": int(info[-1])}
